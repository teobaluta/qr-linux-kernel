...
static int Split_eatKanji(const char *string, QRinput * input,
			  QRencodeMode hint);
			  ...
			  QRencodeMode hint)
{
	...
	run = p - string;
	ret =
	    QRinput_append(input, QR_MODE_KANJI, run, (unsigned char *)string);
	if (ret < 0)
		return -1;
